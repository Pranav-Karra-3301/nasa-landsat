...
const LEGACY_OG_IMAGE = path.join(process.cwd(), "public", "og-image-legacy.png");
...
test("static OG image has the expected preview dimensions and size", async () => {
  ...
  assert.equal(metadata.width, 1131);
  assert.equal(metadata.height, 954);
  assert.equal(metadata.format, "png");
  assert.ok(size <= MAX_BYTES, `Expected ${size} bytes to be at most ${MAX_BYTES}`);
});

test("legacy OG image is preserved separately", async () => {
  const metadata = await sharp(LEGACY_OG_IMAGE).metadata();
